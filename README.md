# 🏨 Business Hotel Website

live demo - 

A responsive **Business Hotel Website** built using **HTML, CSS, and JavaScript** as part of a Front-End Developer assignment.  
The project focuses on clean UI, business-oriented design, and responsiveness across devices.

---

## 📄 Pages Included

- **Home Page**
  - Hero banner with business hotel positioning
  - About hotel section
  - Amenities (Wi-Fi, Conference Room, Dining, etc.)
  - Featured rooms section

- **Rooms Page**
  - Minimum 3 room types
  - Room cards with price per night
  - “Book Now” button (UI only)

- **Contact Page**
  - Contact form
  - Hotel address and contact information
  - Embedded Google Map

---

## 🛠️ Tech Stack

- **HTML5** – Page structure
- **CSS3** – Styling, layout, responsiveness
- **JavaScript** – Basic interactivity
- **VS Code** – Development environment

---

## 📱 Features

- Fully responsive design (Desktop, Tablet, Mobile)
- Clean and modern business hotel UI
- Reusable CSS classes and variables
- Smooth hover effects and animations
- Organized folder structure

---

## 📁 Project Structure

business-hotel-website/
│
├── index.html
├── rooms.html
├── contact.html
│
├── css/
│ └── style.css
│
├── js/
│ └── main.js
│
└── README.md


---

## 🚀 How to Run Locally

1. Clone the repository:
   ```bash
   git clone https://github.com/Nishantbro/business-hotel-website.git
 
 
 👨‍💻 Author

Nishant Sharma
Front-End Developer

✨ Built with focus on clarity, responsiveness, and professional UI standards.


---

## ✅ Next move
- Paste this in `README.md`
- `git add README.md`
- `git commit -m "Added project README"`
- `git push`

If you want, next I can:
- Review README like HR 👀  
- Help zip + submission checklist 📦  
- Draft submission mail to HR 📧  

Bas bolo — **“ready to submit”** 🚀